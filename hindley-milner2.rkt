...
; type-vars-in now handles arbitrary type schemes, and hence is called free-type-vars-in. It also
; takes an additional parameter that specifies a set of variables to treat as constants (and hence
; exclude from the list).
(define/contract (free-type-vars-in α [consts '()])
  (->* (type?) ((listof symbol?)) (listof symbol?))
  (match α
    [(? (curryr member consts) a) '()]
    [(? symbol? a)                (list a)]
    [`(→ ,A ,B)                   (remove-duplicates (append (free-type-vars-in A consts)
                                                             (free-type-vars-in B consts)))]
    [`(∀ ,a ,α)                   (free-type-vars-in α (cons a consts))]))

(define/contract (type-vars-in A)
  (-> type? (listof symbol?))
  (free-type-vars-in A))

(define/contract (closure A [consts '()])
  (->* (type?) ((listof symbol?)) type-scheme?)
  (let loop ([as (free-type-vars-in A consts)] [α A])
    (match as
      ['()             α]
      [(list a as ...) `(∀ ,a ,(loop as α))])))
...
(struct exn:unification-failure exn:fail ())

(define (raise-unification-failure msg lhs rhs)
  (raise (exn:unification-failure (format (string-join
                                           '("cannot unify equation;"
                                             "  ~a"
                                             "    left-hand side: ~a"
                                             "    right-hand side: ~a")
                                           "\n")
    ...
    [(list `(= ,A ,B) equations ...)
     (match* (A B)
       ...
       [((? symbol? a) A)       (if (occurs-in? a A)
                                    (raise-unification-failure
                                     "type variable not unifiable with type it occurs in"
                                     a A)
                                    (let ([s (hasheq a A)])
                                      (unify
...
(define/contract (fresh-inst α)
  (-> type-scheme? type?)
  (let loop ([s (hasheq)] [α α])
    (match α
      [`(∀ ,a ,α) (loop (hash-set s a (gensym)) α)]
      [A          (subst-app s A)])))
...
(define env? (hash/c symbol? type?))

(define/contract (infer-context-for t [cont '()] [env (hash)])
  (->* (term?) (cont? env?) context?)
  (match t
    [(? symbol? x)
     (list
      (let ([A (hash-ref env x #f)])
        (if A
            (cons cont (fresh-inst A))
            (cons x (gensym)))))]
    [`((λ ,x ,t) ,u)
     (let ([t-cont (cont-app-cont cont `((λ ,x ()) ,u))]
       ...
       (let ([Γ (infer-context-for u u-cont env)])
         ...
         (let* ([A (type-of u Γ u-cont (list->set (hash-keys env)))]
                [A (closure A (append-map free-type-vars-in (append (dict-values Γ) (hash-values env))))])
           (displayln A)
           (let ([env (hash-set env x A)]) ; g43818 => g43817 is the type of u, and we want to make g43818 polymorphic but not g43817
             ; i.e. A should be ∀ g43818  g43818 => g43817
...
(module+ test
  (check-exn exn:unification-failure?
             (thunk (infer-type-of `(λ x ,(-let 'Kx '(λ y x) '((Kx x) x)))))))
